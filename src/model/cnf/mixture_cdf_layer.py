...
class MixtureCDFCoupling(CouplingLayer):
    ...
    def forward(self,
        ...
        t, log_s, log_pi, mixt_t, mixt_log_s = MixtureCDFCoupling.get_mixt_params(
            nn_out,
            mask,
        ...
        z_out, ldj, reg_ldj = MixtureCDFCoupling.run_with_params(
            orig_z=orig_z,
            t=t,
            log_s=log_s,
            log_pi=log_pi,
    ...
    def run_with_params(orig_z,
        ...
        if not reverse:
            ...
            z_out = mixture_log_cdf(x=orig_z,
                                    prior_logits=log_pi,
                                    means=mixt_t,
                                    log_scales=mixt_log_s).exp()

            # Regularize mixtures if wanted (only done during training as LDJ is increased)
            if reg_max > 0 and is_training:
                reg_ldj = torch.stack(
                    [safe_log(z_out), safe_log(1 - z_out)], dim=-1) / np.log(
                        10)  # Change to 10 base
            ...
            logistic_ldj = mixture_log_pdf(orig_z,
                                           prior_logits=log_pi,
                                           means=mixt_t,
                                           log_scales=mixt_log_s)
            # Combine all LDJs
            ldj = (change_mask * (log_s + mixt_ldj + logistic_ldj +
                                  reg_ldj * reg_factor)).sum(dim=[1, 2])
        else:
            ...
            z_out = mixture_inv_cdf(z_out,
                                    prior_logits=log_pi,
                                    means=mixt_t,
                                    log_scales=mixt_log_s)
            # Determine LDJ of this transformation
            logistic_ldj = mixture_log_pdf(z_out,
                                           prior_logits=log_pi,
                                           means=mixt_t,
                                           log_scales=mixt_log_s)
            # Combine all LDJs (note the negative sign)
            ldj = -(change_mask *
                    (log_s + mixt_ldj + logistic_ldj)).sum(dim=[1, 2])
    ...
    @staticmethod
    def get_mixt_params(nn_out,
                        mask,
                        num_mixtures,
                        scaling_factor=None,
                        mixture_scaling_factor=None):
        # Split network output into transformation parameters
        param_num = 2 + num_mixtures * 3
        nn_out = nn_out.reshape(nn_out.shape[:-1] +
                                (nn_out.shape[-1] // param_num, param_num))
        t = nn_out[..., 0]
        log_s = nn_out[..., 1]
        ...
        if scaling_factor is not None:
            scaling_fac = scaling_factor.exp().view(
                *(tuple([1 for _ in range(len(log_s.shape) - 1)]) +
                  scaling_factor.shape))
            log_s = torch.tanh(
                log_s / scaling_fac.clamp(min=1.0)) * scaling_fac
        if mixture_scaling_factor is not None:
            mixt_fac = mixture_scaling_factor.exp().view(
                *(tuple([1 for _ in range(len(mixt_log_s.shape) - 2)]) +
                  mixture_scaling_factor.shape))
            mixt_log_s = torch.tanh(
                mixt_log_s / mixt_fac.clamp(min=1.0)) * mixt_fac
        ...
        if mask is not None:
            ...
            log_pi = log_pi * (1 - mask_ext
                               )  # Not strictly necessary but done for safety
...
def mixture_inv_cdf(y,
    ...
    def body(x_, lb_, ub_):
        cur_y = torch.exp(mixture_log_cdf(x_, prior_logits, means, log_scales))
...
if __name__ == '__main__':
    ...
    model_func = lambda c_out: nn.Sequential(nn.Linear(
        c_in, hidden_size), nn.ReLU(), nn.Linear(hidden_size, c_out))
    mask = CouplingLayer.create_channel_mask(c_in)
    coupling_layer = MixtureCDFCoupling(c_in=c_in,
                                        mask=mask,
                                        model_func=model_func,
                                        block_type="Linear net",
