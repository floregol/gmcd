...
import numpy as np
import sys
from general.mutils import create_channel_mask
from layers.cnf.flow_layer import FlowLayer
from layers.cnf.mixture_cdf_layer import MixtureCDFCoupling
...
class AutoregressiveMixtureCDFCoupling(FlowLayer):
    ...
    def forward(self, z, ldj=None, reverse=False, **kwargs):
        ...
        if not reverse:
            ...
            z = z.double()
            z_out, ldj_mixt = MixtureCDFCoupling.run_with_params(
                z, t, log_s, log_pi, mixt_t, mixt_log_s, reverse=reverse)
        else:
...
if __name__ == "__main__":
    ...
    module = AutoregressiveMixtureCDFCoupling(c_in=c_in, hidden_size=hidden_size, num_mixtures=4,
                                              time_embed=time_embed, autoreg_hidden=True)

    orig_out, _ = module(z=_inp, length=lengths,
                         channel_padding_mask=channel_padding_mask)
    print("Out", orig_out)

    _inp[0, 1, 1] = 10
    alt_out, _ = module(z=_inp, length=lengths,
                        channel_padding_mask=channel_padding_mask)
