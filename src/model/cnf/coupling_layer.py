...
class CouplingLayer(FlowLayer):
    ...
    def _prepare_mask(self, mask, z):
        ...
        mask = self.mask.unsqueeze(dim=0) if len(
            z.shape) > len(self.mask.shape) else self.mask
        if mask.size(1) < z.size(1) and mask.size(1) > 1:
            mask = mask.repeat(
                1, int(math.ceil(z.size(1)/mask.size(1))), 1).contiguous()
    ...
    def create_channel_mask(c_in, ratio=0.5, mask_floor=True):
        ...
        mask = torch.cat(
            [torch.ones(1, c_masked), torch.zeros(1, c_unmasked)], dim=1)
        return mask
    ...
    def create_chess_mask(seq_len=2):
        ...
        mask = torch.cat([torch.ones(seq_mask, 1), torch.zeros(
            seq_unmask, 1)], dim=1).view(-1, 1)
        return mask
    ...
    def info(self):
        ...
        info_str += ", mask ratio %.2f, %s mask" % (
            (1-self.mask).mean().item(), "channel" if is_channel_mask else "chess")
        return info_str
