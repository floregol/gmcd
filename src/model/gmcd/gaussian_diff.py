...
import numpy as np
import torch as th
from src.model.gmcd.diff_utils import *
...
look_sample_process
    ...
    def __init__(
        ...
        super().__init__()
        self.d_in = sequence_length*latent_dim
        self.T = T
    ...
    def p_mean_variance(
        ...
        B, C = x.shape[:2]
        assert t.shape == (B,)
        model_output = self.denoise_fn(x, self._scale_timesteps(t))

        if self.model_var_type in [ModelVarType.LEARNED, ModelVarType.LEARNED_RANGE]:
            assert model_output.shape == (B, C * 2)
    ...
    def _predict_xstart_from_eps(self, x_t, t, eps):
        ...
        return (
            ...
            - extract_into_tensor(self.sqrt_recipm1_alphas_cumprod,
                                  t, x_t.shape) * eps
        )
    ...
    def _vb_terms(
        ...
        decoder_nll = sum_flat(decoder_nll)
    ...
    def forward(self, z, ldj=None, reverse=False, x_cat=None, **kwargs):
        ...
        if not reverse:
            ...
            if self.training:
                ...
                ldj = -self.training_losses(z, t, x_cat)['loss']

            else:
                ldj = -self.calc_bpd_loop(z)['total_vb']
    ...
    def info(self):
        return 'V2 Diffusion', self.model_mean_type, self.model_var_type, self.loss_type
    ...
    def calc_bpd_loop(self, x_start, clip_denoised=True, model_kwargs=None):
        ...
        for t in list(range(self.num_timesteps))[::-1]:
            ...
            xstart_mse.append(sum_flat((out["pred_xstart"] - x_start) ** 2))
            eps = self._predict_eps_from_xstart(
                x_t, t_batch, out["pred_xstart"])
            mse.append(sum_flat((eps - noise) ** 2))

        vb = th.stack(vb, dim=1)
        xstart_mse = th.stack(xstart_mse, dim=1)
        mse = th.stack(mse, dim=1)

        prior_vb = self._prior(x_start)
        total_vb = vb.sum(dim=1) + prior_vb
        return {
            "total_vb": total_vb,
            "prior_vb": prior_vb,
            "vb": vb,
