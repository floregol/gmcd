...
class TaskSyntheticModeling(TaskTemplate):
    ...
    def _load_datasets(self):
        if self.run_config.dataset == 'sort' or self.run_config.dataset == 'pair':
            ...
            self.train_dataset = SyntheticDataset(
                ...
                num_resample=self.run_config.num_resample,
                train=True)
            ...
            self.test_dataset = SyntheticDataset(
                ...
                num_resample=self.run_config.num_resample,
                test=True)
        elif self.run_config.dataset == 'real' or self.run_config.dataset == 'proxy':
            self.train_dataset = RealDataset(
                dataset_name=self.run_config.dataset, train=True)
            self.val_dataset = RealDataset(
                dataset_name=self.run_config.dataset, val=True)
            self.test_dataset = RealDataset(
                dataset_name=self.run_config.dataset, test=True)
